...
bool escapePressed = false; // added flag for escape key
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "0432310005101056", NULL, NULL); // changed window title
    if (window == NULL)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (escapePressed)
        {
            glUniform4f(vertexColorLocation, 1.0f, 0.0f, 0.0f, 1.0f); // red if escape pressed
        }
        else
        {
            float timeValue = glfwGetTime();
            float redValue = (sin(timeValue) / 2.0f) + 0.5f; // oscillates smoothly 0 -> 1
            glUniform4f(vertexColorLocation, redValue, 1.0f, 1.0f, 1.0f); // cyan <-> white only
        }
...
void processInput(GLFWwindow *window)
{
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
        escapePressed = true; // set flag instead of closing
}
